/*
  ...
  Example task 3; a task that writes a counter to STDOUT every few seconds (specified by TIMERINTERVAL, triggered by the software timer). 
...
#define EVENTID       222   // ID for the repetitive timer
                ...
                while((TISM_PostmanMessagesWaiting(ThisTask)>0) && (MessageCounter<MAX_MESSAGES))
                {
                  ...
                  switch(MessageToProcess->MessageType)
                  {
                    case TISM_PING:          // Check if this process is still alive. Reply with a ECHO message type; return same message payload.
                                             ...
                                             break;
                    case EVENTID:            // Timer expired; write the number of runs in the interval to STDOUT.
                                             fprintf(STDOUT, "%s: Number of runs in this cycle: %d.\n", ThisTask.TaskName, ExampleTask3Data.NumberOfRunsCounter);
                                             ExampleTask3Data.NumberOfRunsCounter=0;
                                             break;
                    case GPIO_IRQ_EDGE_FALL: // Button is pressed (message from ExampleTask1). Increase this task's priority.
                            		             if (ThisTask.TaskDebug) fprintf(STDOUT, "%s: Message received; button pressed.\n", ThisTask.TaskName);
                    ...
                    case GPIO_IRQ_EDGE_RISE: // Button is released (message from ExampleTask1). Set this task's priority to normal.
                                             if (ThisTask.TaskDebug) fprintf(STDOUT, "%s: Message received; button released.\n", ThisTask.TaskName);
