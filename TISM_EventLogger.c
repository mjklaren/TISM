...
uint8_t TISM_EventLogger (TISM_Task ThisTask)
{
  ...
  switch(ThisTask.TaskState)   
  {
    case INIT:  // Activities to initialize this task (e.g. initialize ports or peripherals).
	  ...
	  case RUN:   // Do the work.						
                ...
                {
                  fprintf(STDOUT, "%llu %s (T:%d,H:%d): Doing work with priority %d on core %d.\n", time_us_64(), ThisTask.TaskName, ThisTask.TaskID, System.HostID, ThisTask.TaskPriority, ThisTask.RunningOnCoreID);
                  fflush(STDOUT);
                }
                ...
                {        
                  ...
                  {
                    fprintf(STDOUT, "%llu %s (T:%d,H:%d): Message '%ld' type %d from TaskID %d (HostID %d) received.\n", time_us_64(), ThisTask.TaskName, ThisTask.TaskID, System.HostID, MessageToProcess->Payload0, MessageToProcess->MessageType, MessageToProcess->SenderTaskID, MessageToProcess->SenderHostID);
                    fflush(STDOUT);
                  }
                  ...
                  switch(MessageToProcess->MessageType)
                  {
                    ...
                    case TISM_LOG_EVENT_NOTIFY: // Log event message; write the message to STDOUT.
                                                // Logging from other hosts is very limited at this time; Payload0 and Payload1 in a TISM message have 8 bytes in total available.
                                                if(MessageToProcess->SenderHostID==System.HostID)
                                                {
                                                  fprintf(STDOUT, "%llu %s (T:%d,H:%d): %s\n", MessageToProcess->MessageTimestamp, System.Task[MessageToProcess->SenderTaskID].TaskName, MessageToProcess->SenderTaskID, MessageToProcess->SenderHostID, (char *)MessageToProcess->Payload0);
                                                  free((char *)MessageToProcess->Payload0);
                                                }
                                                else
                                                  fprintf(STDOUT, "%llu %s (T:%d,H:%d): %s%s\n", MessageToProcess->MessageTimestamp, System.Task[MessageToProcess->SenderTaskID].TaskName, MessageToProcess->SenderTaskID, MessageToProcess->SenderHostID, MessageToProcess->Payload0, MessageToProcess->Payload1);
                                                fflush(STDOUT); 
                                                break;
	  ...
	  case STOP:  // Task required to stop this task, including the last log entry.
                fprintf(STDOUT, "%llu %s (T:%d,H:%d): Logging stopped.\n", time_us_64(), ThisTask.TaskName, ThisTask.TaskID, System.HostID);
                fflush(STDOUT);
  ...
  // Run completed.
  if (ThisTask.TaskDebug==DEBUG_HIGH) fprintf(STDOUT, "%llu %s (T:%d): Run completed.\n", time_us_64(), ThisTask.TaskName, ThisTask.TaskID);
