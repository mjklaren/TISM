...
static struct TISM_IRQHandlerDataGPIO GPIO[NUMBER_OF_GPIO_PORTS];
...
{
  // Use the Payload1-field in the message to capture the AntiBounceTimeout and the GPIOPullDown.
...
uint8_t TISM_IRQHandler (TISM_Task ThisTask)
{
  ...
  switch(ThisTask.TaskState)   
  {
    case INIT:  // Task required to initialize                
                ...
                {
                  GPIO[counter].Initialized=false;
                  GPIO[counter].GPIOPullDown=true;                  
                  GPIO[counter].EventMask=0;
                  GPIO[counter].Subscriptions=NULL;
                }
	  ...
	  case RUN:   // Do the work						
                ...
                while((TISM_PostmanMessagesWaiting(System.IRQHandlerInboundQueue)>0) && (MessageCounter<IRQHANDLER_MAX_MESSAGES))
                {
                  ...
                  if (ThisTask.TaskDebug) TISM_EventLoggerLogEvent (ThisTask, TISM_LOG_EVENT_NOTIFY, "Processing interrupt message '%ld' with type %s-0x%02X from the IRQ handler queue.", MessageToProcess->Payload0, TISM_MessageTypeToString(MessageToProcess->MessageType), MessageToProcess->MessageType);

                  if(GPIO[MessageToProcess->MessageType].Initialized)
                  {                
                    struct TISM_IRQHandlerSubscription *SearchPointer=GPIO[MessageToProcess->MessageType].Subscriptions;
                    while(SearchPointer!=NULL)
                    {
                      if(MessageToProcess->Payload0 & SearchPointer->Events)
                      {
                        ...
                        {
                          TISM_PostmanTaskWriteMessage(ThisTask,SearchPointer->HostID,SearchPointer->TaskID,MessageToProcess->MessageType,MessageToProcess->Payload0,GPIO[MessageToProcess->MessageType].GPIOPullDown);                          
                ...
                while((TISM_PostmanTaskMessagesWaiting(ThisTask)>0) && (MessageCounter<MAX_MESSAGES))
                {
                  ...
                  switch(MessageToProcess->MessageType)
                  {
                    ...
                    case GPIO_28:   // Subscription request received; register or update.
                                    ...
                                    if(GPIO[MessageToProcess->MessageType].Initialized==false)
                                    {
                                      // Bug fix; when we receive an IRQ_UNSUBSCRIBE-request to an uninitialized port.
                                      if(MessageToProcess->Payload0==IRQ_UNSUBSCRIBE)
                                      {
                                      ...
                                      // Is the 'pull down' bit set in the Payload1-field?
                                      if((MessageToProcess->Payload1 & 0x01000000)==0x01000000)
                                      {
                                        gpio_pull_down(MessageToProcess->MessageType);
                                        GPIO[MessageToProcess->MessageType].GPIOPullDown=true;
                                      }
                                      else
                                      {
                                        gpio_pull_up(MessageToProcess->MessageType);
                                        GPIO[MessageToProcess->MessageType].GPIOPullDown=false;
                                      }

                                      if (ThisTask.TaskDebug==DEBUG_HIGH) TISM_EventLoggerLogEvent (ThisTask, TISM_LOG_EVENT_NOTIFY, "First subscription, GPIO %d initialized (request from TaskID %d, HostID %d, event %d, internal resistor pull-%s).", MessageToProcess->MessageType, MessageToProcess->SenderTaskID, MessageToProcess->SenderHostID, MessageToProcess->Payload0, (GPIO[MessageToProcess->MessageType].GPIOPullDown==false?"up":"down"));

                                      GPIO[MessageToProcess->MessageType].Initialized=true;

                                      // Now register the first subscription in the linear list.
                                      GPIO[MessageToProcess->MessageType].Subscriptions=(struct TISM_IRQHandlerSubscription *) malloc(sizeof(struct TISM_IRQHandlerSubscription));
                                      if(GPIO[MessageToProcess->MessageType].Subscriptions==NULL)
                                      {
                                        // Memory allocation failed
                                        TISM_EventLoggerLogEvent(ThisTask, TISM_LOG_EVENT_ERROR, "Failed to allocate memory to register new subscription for GPIO %d.", MessageToProcess->MessageType);
                                        break;
                                      }
                                      else
                                      {
                                        GPIO[MessageToProcess->MessageType].Subscriptions->HostID=MessageToProcess->SenderHostID;
                                        GPIO[MessageToProcess->MessageType].Subscriptions->TaskID=MessageToProcess->SenderTaskID;
                                        GPIO[MessageToProcess->MessageType].Subscriptions->Events=MessageToProcess->Payload0;
                                        GPIO[MessageToProcess->MessageType].Subscriptions->AntiBounceTimeout=(MessageToProcess->Payload1 & 0xFFFFFF);
                                        GPIO[MessageToProcess->MessageType].Subscriptions->LastSuccessfullInterrupt=0;
                                        GPIO[MessageToProcess->MessageType].Subscriptions->NextSubscription=NULL;
                                      }
                                    }
                                    else
                                    {
                                      // There are already tasks subscribed to this GPIO. 
                                      if (ThisTask.TaskDebug==DEBUG_HIGH) TISM_EventLoggerLogEvent (ThisTask, TISM_LOG_EVENT_NOTIFY, "Subscription to GPIO %d being added or modified (TaskID %d, HostID %d, event %d).", MessageToProcess->MessageType, MessageToProcess->SenderTaskID, MessageToProcess->SenderHostID, MessageToProcess->Payload0);

                                      // Find the entry for this TaskID in the linear list, or create a new one.
                                      SearchPointer=GPIO[MessageToProcess->MessageType].Subscriptions;
                                      ...
                                      if(SearchPointer!=NULL)
                                      {
                                        ...
                                        if(MessageToProcess->Payload0==IRQ_UNSUBSCRIBE)
                                        {
                                          ...
                                          if(SearchPointer==PreviousSearchPointer)
                                          {
                                            ...
                                            GPIO[MessageToProcess->MessageType].Subscriptions=SearchPointer->NextSubscription;
                                          }
                                          else
                                          {
                                        ...
                                        else
                                        {
                                          ...
                                          SearchPointer->Events=MessageToProcess->Payload0;
                                      ...
                                      else
                                      {
                                        ...
                                        {
                                          NewSubscription->HostID=MessageToProcess->SenderHostID;
                                          NewSubscription->TaskID=MessageToProcess->SenderTaskID;
                                          NewSubscription->Events=MessageToProcess->Payload0;
                                          NewSubscription->AntiBounceTimeout=(MessageToProcess->Payload1 & 0xFFFFFF);
                                    ...
                                    if(GPIO[MessageToProcess->MessageType].Subscriptions!=NULL)
                                    {
                                      ...
                                      GPIO[MessageToProcess->MessageType].EventMask=TISM_IRQHandlerCalculateEventsMask(GPIO[MessageToProcess->MessageType].Subscriptions);

                                      // (Re)register our interrupt-handler to this GPIO.
                                      gpio_set_irq_enabled_with_callback(MessageToProcess->MessageType, GPIO[MessageToProcess->MessageType].EventMask, true, (void*)&TISM_IRQHandlerCallback);
                                    }
                                    else
                                    {
                                      // No subscriptions left. Set the eventmask to 0 and disable all the interrupt events for this GPIO.  
                                      gpio_set_irq_enabled(MessageToProcess->MessageType, GPIO[MessageToProcess->MessageType].EventMask, false);
                                      GPIO[MessageToProcess->MessageType].EventMask=0; 
                                      GPIO[MessageToProcess->MessageType].Initialized=false;

                                      if(ThisTask.TaskDebug) TISM_EventLoggerLogEvent(ThisTask, TISM_LOG_EVENT_NOTIFY, "GPIO %d: all subscriptions removed, interrupt disabled.", MessageToProcess->MessageType);
                                    }
		                                ...
		                                if (ThisTask.TaskDebug) 
                                    {
                                      ...
                                      TISM_EventLoggerLogEvent (ThisTask, TISM_LOG_EVENT_NOTIFY, "List complete. Summary event mask: %ld. GPIO initialized as %s.", GPIO[MessageToProcess->MessageType].EventMask, (GPIO[MessageToProcess->MessageType].GPIOPullDown?"pull-down":"pull-up"));
                                    }
                                    break;
